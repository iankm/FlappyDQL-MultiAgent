...
class DeepQPongPlayer(PongPlayer):
    ...
    OBSERVATION = 50000.  # time steps to observe before training
    EXPLORATION = 500000.  # frames over which to anneal epsilon
    I_EPSILON = 1.0  # starting chance of an action being random
    F_EPSILON = 0.05  # final chance of an action being random
    REMEMBER = 500000  # number of observations to remember
    ...
    def __init__(self, checkpoint_path="deep_q_pong_networks", playback_mode=False, verbose_logging=False):
        ...
        self._probability_of_random_action = self.I_EPSILON
        self._time = 0
    ...
    def get_keys_pressed(self, screen_array, reward, terminal):
        ...
        if not self._playback_mode:
            ...
            if len(self._observations) > self.REMEMBER:
        ...
        if not self._playback_mode:
            ...
            if self._probability_of_random_action > self.F_EPSILON \
                    and len(self._observations) > self.OBSERVATION:
                self._probability_of_random_action -= \
                    (self.I_EPSILON - self.F_EPSILON) / self.EXPLORATION
        ...
        for i in range(len(mini_batch)):
            if mini_batch[i][self.OBS_TERMINAL_INDEX]:
                ...
                agents_expected_reward.append(
                    rewards[i] + self.GAMMA * np.max(agents_reward_per_action[i]))
